...
function connectToSW(): void {
  ...
  try {
    port = chrome.runtime.connect({ name: "chatpulse-content" });
...
function handleChatEvent(event: CustomEvent<{ frame: string; timestamp: number }>): void {
  ...
  const parsed = parseTwitchIRC(frame);
  if (!parsed) {
    console.log(TAG, "Frame parse returned null, skipping");
    return;
  }

  const unified = toUnifiedChatMessage(parsed, timestamp);
...
window.addEventListener("chatpulse:msg", handleChatEvent as EventListener);
